...
Elevator::Elevator(int level, int max)
{
	currentLevel = level;
	nextLevel = level;
	currentPeople = 0;
...
void Elevator::addTask(Task task, int t)
{
...
vector<Task> Elevator::getTasks()
{
	return tasks;
};
void Elevator::task(int next, int people )
{
	if (currentLevel != next)
		nextLevel = next;
	currentPeople = people;
};
void Elevator::task(bool stop)
{
	if (tasks[0][4] <= 0 || stop)
		tasks.erase(tasks.begin());
	if (!tasks.empty())
...
int Elevator::getCounter()
{
	return counter;
};
void Elevator::exit(int ep)
{
	counter = 1;
	if (ep = 0)
	{
